...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        ...
        const emissiveColor = uniform( color( '#3c8e06' ) );
		const timeScale = uniform( 0.1 );
		const parabolStrength = uniform( 2 );
		const parabolOffset = uniform( 1 );
		const parabolAmplitude = uniform( 1.29 );
		...
		controls.target.x = -0.04;
		controls.target.y = 0.84;
...
// 
        ...
        // controlsGui.add( controls.target, 'x', -10 , 10, 0.04  ).name( 'target.x');
        // controlsGui.add( controls.target, 'y', -10 , 10, 0.04  ).name( 'target.y');
        // controlsGui.add( controls.target, 'z', -10 , 10, 0.04  ).name( 'target.z');
// 
        // const cameraGui = gui.addFolder('camera');
        // cameraGui.add( camera.position, 'x', -10, 10, 0.04).name('camera.position.x');
        // cameraGui.add( camera.position, 'y', -10, 10, 0.04).name('camera.position.y');
        // cameraGui.add( camera.position, 'z', -10, 10, 0.04).name('camera.position.z');
